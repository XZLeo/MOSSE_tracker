...
def crop_patch2(image, region, img_channels):
    """
    ...
    patch = np.zeros(shape=(region.height, region.width, img_channels),
                     dtype=image.dtype)

    patch[rp0:rp1, cp0:cp1] = image[ri0:ri1, ci0:ci1]

    assert patch.shape == (region.height, region.width, img_channels)
    return patch
...
def read_image(image_path, transform=None):
    ...
    if file_ending == '.pfm':
        ...
        raise NotImplementedError("No way to read files of type: {}".format(file_ending))
